...
export default function AdminCreateClassPage() {
  ...
  const handleCreateCourse = async () => {
    ...
    setIsCreating(true);
    try {
      const course = await classroomService.createCourse(token, newCourse);
      
      // If teacher email is provided, invite them to the course
      if (teacherEmail.trim()) {
        try {
          await classroomService.inviteTeacher(token, course.id, teacherEmail.trim());
          toast.success('Course created and teacher invited!');
        } catch (inviteError: any) {
          toast.success('Course created!');
          toast.error(`Failed to invite teacher: ${inviteError.message}`);
        }
      } else {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Set up a new class and optionally assign a teacher</p>
      </div>
          ...
          <div className="border-t pt-4 mt-4">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Enter the teacher's email to invite them as a co-teacher
              </p>
            </div>
          </div>
